...
# def smooth(true_labels: torch.Tensor, classes=len(CATEGORY_ID_LIST), smoothing=0.1):
#         """
...
#         label_shape = torch.Size((true_labels.shape[0], classes))    # torch.Size([2, 5])
#         with torch.no_grad():
#             true_dist = torch.empty(size=label_shape, device=true_labels.device)    # 空的，没有初始化
#             true_dist.fill_(smoothing / (classes - 1))
#             _, index = torch.max(true_labels, torch.ones)
